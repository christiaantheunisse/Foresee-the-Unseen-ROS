...
########################################## Store a created map ############################################
### $ ros2 run nav2_map_server map_saver_cli -f ~/thesis/robot_ws/src/foresee_the_unseen/maps/some_name ###
###########################################################################################################


### If defined, the ros maps are obtained from `ROS_MAP_FILES_DIR`
...
def generate_launch_description():
    ...
    map_file_name_launch_arg = DeclareLaunchArgument("map_file_name", default_value=TextSubstitution(text="on_the_floor"))
    ...
    try:
        ...
        log_messages.append(LogInfo(msg="The bag file is loaded from the path in `ROS_BAG_FILES_DIR`"))
    except KeyError:
        log_messages.append(LogInfo(msg="`ROS_BAG_FILES_DIR` is not set!"))
        bag_files_dir = ""
    ...
    slam_localizing_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
        ...
        launch_arguments={
            "slam_params_file": PathJoinSubstitution(
                ...
                ["/home/christiaan/thesis/robot_ws/src/foresee_the_unseen/config/mapper_params_localization.yaml"]
            ),
            "map_file_name": PathJoinSubstitution([map_files_dir, map_file_name]),
        }.items(),
        condition=UnlessCondition(record_rosbag),  # not record_rosbag
    )
