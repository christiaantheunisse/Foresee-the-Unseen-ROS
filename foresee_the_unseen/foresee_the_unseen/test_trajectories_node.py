...
MARGIN_VELOCITY = 0.0001  # [m/s]
MARGIN_POSITION = 0.2  # [m/s]
MARGIN_ORIENTATION = 0.2  # [rad]
REFERENCE_SPEED = 0.1 # [m/s]

# static transfrom command line:
# ros2 run tf2_ros static_transform_publisher --x 1.0 --y -0.2 --frame-id map --child-frame-id test_traject
...
PlannerStates: TypeAlias = Literal["initialized", "returning to boundary", "following trajectory"]
...
class TestTrajectoriesNode(Node):
    """Plans trajectories in the `test_traject` frame."""

    def __init__(self):
        super().__init__("test_trajectories_node")
        ...
        self.publisher_trajectory = self.create_publisher(Trajectory, "trajectory", 5)

        self.waypoints = np.array([[0, 0], [TEST_DISTANCE / 2, 0], [TEST_DISTANCE, 0]])
        ...
        # self.test_straight_trajectories()
        self.create_timer(1 / FREQUENCY_TRAJECTORY, self.test_straight_trajectories)
    ...
    @property
    def trajectory_msg(self) -> Trajectory:
        if self._trajectory_msg is None:
            raise AttributeError
        return self._trajectory_msg
    
    @trajectory_msg.setter
    def trajectory_msg(self, trajectory_msg: Trajectory) -> None:
    ...
    def transform_state(state: State, transform: TransformStamped) -> State:
        ...
        yaw = TestTrajectoriesNode.yaw_from_quaternion(*quaternion)
        translation = [
    ...
    def odom_callback(self, msg: Odometry) -> None:
        ...
        try:
            t_odom_map = self.tf_buffer.lookup_transform(self.traject_frame, odom_frame, RosTime())
        ...
        state = State(position=np.array(position), orientation=yaw, velocity=velocity)
        self.state = self.transform_state(state, t_odom_map)
    ...
    def publish_trajectory(
        ...
        msg = self.create_trajectory_msg(positions, orientations, velocities, time_stamps)
        self.publisher_trajectory.publish(msg)
        time_goal = time_stamps[-1]
    ...
    def velocity_profile_to_state_list(
        ...
        orient_bw_points = TestTrajectoriesNode.make_angles_continuous(orient_bw_points)
        ...
        orientations = TestTrajectoriesNode.normalize_angles(orientations)
        time_stamps = (np.arange(len(velocity_profile)) + 1) * dt

        return positions, orientations, velocity_profile, time_stamps

    @staticmethod
    def get_reposition_trajectory(
        current_pos_difference: npt.NDArray[npt.Shape["2"], npt.Float],
        lin_distance: float = 0.1,
        ...
        pts_total = np.insert(pts_total, 0, -current_pos_difference, axis=0)
        ...
        positions, orientations, velocities, time_stamps = TestTrajectoriesNode.velocity_profile_to_state_list(
            velocity_profile=velocity_profile,
            waypoints=pts_total,
        )

        return positions, orientations, velocities, time_stamps
    ...
    def create_trajectory_msg(
        ...
        pose_stamped_list = []
        twist_list = []
        for position, orientation, velocity, time_stamp in zip(positions, orientations, velocities, time_stamps):
        ...
        return Trajectory(path=Path(header=header_path, poses=pose_stamped_list), velocities=twist_list)
    ...
    def return_to_boundary(self, do_return_to_start: bool) -> None:
        ...
        positions, orientations, velocities, time_stamps = self.get_reposition_trajectory(pt_goal - pt_current)
        positions, orientations = self.transform_positions_orientations(
            positions,
            orientations,
            translation=self.waypoints[idx_waypoint],
            rotation=self.orient_bw_points[0] + (np.pi if do_return_to_start else 0),
        )

        # Publish the trajectory
        self.publish_trajectory(positions, orientations, velocities, time_stamps)
    ...
    def has_goal_position(self) -> bool:
        return bool(
            ...
            and np.abs(self.state.orientation - self.goal_state.orientation) < MARGIN_ORIENTATION
        )

    def has_goal_velocity(self) -> bool:
        return bool(np.abs(self.state.velocity - self.goal_state.velocity) < MARGIN_VELOCITY)

    def test_straight_trajectories(self) -> None:
        """The function that runs the straight line tests along the positive x-axis."""
        try:
            self.state
        except AttributeError:
        ...
        is_at_start = True  # true if traveling in the positive x direction, otherwise False
        if self.planner_state == "initialized":
            self.return_to_boundary(do_return_to_start=not is_at_start)
            is_at_start = not is_at_start
            self.planner_state = "returning to boundary"
            self.get_logger().info(f"State changed to {self.planner_state}")
...
def main(args=None):
    ...
    test_trajectories_node = TestTrajectoriesNode()

    rclpy.spin(test_trajectories_node)
    test_trajectories_node.destroy_node()
    rclpy.shutdown()
